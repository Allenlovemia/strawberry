...
result <- drop_one_value_col(strawberry)

if (is.data.frame(result)) {
  cols_to_drop <- result$col_name
  strawberry <- strawberry |> select(!all_of(cols_to_drop))
  print(result |> kable(caption = "Dropped Single-Value Columns: names and values"))
} else {
  # Handle the case where there are no columns to drop
  print(result)
}
...
```
2. Distribution of Entries by Year:
```{r}
